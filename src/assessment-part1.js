...
String.prototype.grammarPolice = function() {
  ...
  strArr.forEach(function(e) {
    var rString = "";
    rString = e.slice(0,1).toUpperCase() + e.slice(1).toLowerCase();
    nString.push(rString);
  });
  var lastString = nString.join(" ");
  return lastString;
};
...
function valueType(param1, param2) {
  ...
  if (param1 == param2 && param1 !== param2) return "Same value, different types";
...
function promiseCatcher(param) {
  param.then(function(results) {theAnswer = results});
}
